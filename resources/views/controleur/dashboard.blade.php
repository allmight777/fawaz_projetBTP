...
@section('title', 'Dashboard - Bureau de Contrôle Collaborateur')
...
<style>
    ...
    .stat-card:hover {
        ...
        box-shadow: 0 8px 25px rgba(4,120,87,0.12);
    }
    ...
    .stat-card .stat-icon.green-bg { background: rgba(4,120,87,0.1); color: #047857; }
    ...
    .lot-banner .lot-info p {
        color: #555;
        font-size: 13px;
        margin: 2px 0 0 0;
    }

    .lot-banner .lot-info p i {
        color: #047857;
    }
...
<div class="lot-banner">
    <div class="lot-info">
        ...
        <div>
            <h3>{{ $lot->nom ?? 'Aucun lot assigné' }}</h3>
            <p>
                <i class="fas fa-building"></i> {{ $user->structure?->nom ?? 'Structure non définie' }}
                @if($user->fonction)
...
<div class="stats-grid">
    <div class="stat-card">
        <div class="stat-info">
            ...
            <div class="stat-number green">{{ $totalDocuments }}</div>
        </div>
        <div class="stat-icon green-bg"><i class="fas fa-file-alt"></i></div>
    </div>

    <div class="stat-card">
        <div class="stat-info">
            <h3>En Attente</h3>
            <div class="stat-number orange">{{ $documentsEnAttente }}</div>
        </div>
    ...
    <div class="stat-card">
        <div class="stat-info">
            <h3>En Cours</h3>
            <div class="stat-number blue">{{ $documentsEnCours }}</div>
        </div>
        <div class="stat-icon blue-bg"><i class="fas fa-spinner"></i></div>
    </div>

    <div class="stat-card">
        <div class="stat-info">
            <h3>Validés</h3>
            <div class="stat-number green">{{ $documentsValides }}</div>
        </div>
    ...
    <div class="stat-card">
        <div class="stat-info">
            <h3>À corriger</h3>
            <div class="stat-number red">{{ $documentsACorriger }}</div>
        </div>
    ...
    <div class="stat-card">
        <div class="stat-info">
            <h3>En Analyse</h3>
            <div class="stat-number blue">{{ $documentsEnAnalyse }}</div>
        </div>
        <div class="stat-icon blue-bg"><i class="fas fa-search"></i></div>
    </div>
</div>
...
<div class="charts-grid">
    <div class="chart-card">
        <div class="chart-header">
            <h4><i class="fas fa-chart-line"></i> Évolution des documents</h4>
    ...
    <div class="chart-card">
        <div class="chart-header">
            ...
            <span class="text-muted">État des documents</span>
        </div>
        <div class="chart-container">
...
<div class="table-container">
    <div class="table-header">
        ...
        <a href="{{ route('controleur.affectations.index') }}" class="btn-link">
    ...
    <div class="table-wrapper">
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <a href="{{ route('controleur.affectations.show', $doc->id) }}" class="btn-link" style="font-weight:600;">
                            #{{ $doc->id }}
                        </a>
                    </td>
                    <td>{{ Str::limit($doc->titre, 40) }}</td>
                    <td>{{ $doc->documentType->nom ?? '-' }}</td>
                    <td>
                        <span class="badge
                            @if($doc->statut == 'valide') badge-valide
                            @elseif($doc->statut == 'en_attente') badge-attente
                            @elseif($doc->statut == 'en_analyse') badge-controle
